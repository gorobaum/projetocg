...
\begin{align}
	p' = p_0 + (p_1-p_0)*\frac{t-t_0}{t_1-t_0}
\end{align}
Where $p$ is the intensities of the Light Probe $p$ and $t$ it's the time of the resulting Light Probe.
...
$(\dots,t_{i-2},t_{i-1},t_i,t_{i+1},t_{i+2},\dots)$ and the capture time $(\dots,p_{i-2},p_{i-1},p_i,p_{i+1},p_{i+2},\dots)$ of each observation, we can interpolate a new
...
\end{split}
\end{align}
	The Foward interpolation uses an iterative method that adds the $n$-esieme central difference\cite{abramowitz1972handbook} to calculate the resulting Light Probe.
The method uses a piramidal 
\section{Experiment}
