...
\subsection{Our data}
	...
	\begin{figure}[!ht]
		...
		\centering
		\includegraphics[width=10cm]{images/lightprobesext.png}
		\label{fig:lpext}
	\end{figure}
...
One thing we noticed when using higher order interpolations like Lagrange and Gauss was that sometimes the pixel intensities would be negative. Because of this at the end of the interpolation, we clamp the values to 0.0 if they are negative, which created black areas in the resulting image. Linear interpolation does not suffer from this effect.
...
\subsection{Rendering the white cube}
	...
	\begin{figure}[!ht]
		...
		\centering
		\includegraphics[width=9cm]{images/base.png}
		\label{fig:base}
	\end{figure}
	...
	\begin{figure}[!ht]
		...
		\centering
		\includegraphics[width=9cm]{images/esf.png}
		\label{fig:esf}
	\end{figure}
	...
	Because there were many variables involved – geometry, materials, and image based lighting coefficient - we were not able to reproduce the shading the cubes should have had. It is worth noting that even though the shading did not match the real cube, the lighting consistency is not totally lost the sense that darker and lighter areas are still mostly same as in the original image. The exception to this is figure X, where the left side of the cube is darker than it should be.
